...
## 0.2 增補:seed 步驟搭在同一個 one-shot task 上

日期:2026-06-14

### 變更

one-shot task 的 command override 從:
...
對應 `backend/package.json` 將 `tsx` 從 devDependencies 移到 dependencies(`prisma db seed` 透過 `package.json` 的 `prisma.seed` 欄位呼叫 `tsx prisma/seed.ts`,runtime image 必須帶 tsx)。

### 為什麼搭在同一個 task

§5 已經把這個 pattern 標示為「VPC-only 維運工作的可重複模板」,seed 是其中一個自然落點:同樣需要連 VPC 內 PG、同樣需要 S3 PutObject(用 ECS task 既有的 execution role)、同樣是 deploy 前要完成的前置作業。

不拆成兩個獨立 RunTask 的理由:Fargate ENI provisioning 每次 30-60 秒,兩個 task 等於額外 1 分鐘 CD 時間;`&&` 串接讓任一步失敗都觸發既有的 exit-code abort 路徑,失敗 semantics 不弱化。代價是 migrate / seed 共用一個 exit code,debug 時靠 log 行區分(`Applied N migrations` vs `→ seeding 16 categories`)。

### 副作用:每次 deploy 都會清空並重塞 donation 域資料

`prisma/seed.ts` 走 deleteMany → re-create 流程(spec 015 §6.4 的 idempotent 重塞策略,原本針對 dev 環境)。搬到 prod deploy 路徑上代表:

- 每次 push 到 main → seed 跑 → charities / donation_projects / sale_items / charity_categories 全部洗掉重建
- charity uuid 每次 deploy 都會換,reviewer 如果書籤了 `/charities/<id>` 連結會在下次 deploy 後失效
- 任何在 prod 透過 API 建出來的 charity 都會在下次 deploy 被擦掉

**此設計只適用於面試 demo 環境**(沒有 admin 後台、沒有「真實」慈善團體會註冊)。production-grade 的 admin flow 出現時必須:

1. 把 seed 從 deploy 路徑搬出來
2. 改用 `workflow_dispatch` 手動觸發 + 確認對話
3. seed 內部加 `if (existingChariesCount > 0) skip` guard
4. 或拆 seed:Category(字典表,migration 等價物) vs Charity(改為 admin 後台建立)

### 必要的 IAM 權限增補

ECS task 的 execution role 必須允許對 prod bucket(`vars.S3_BUCKET`)做 `s3:PutObject` — seed 會上傳 23 + 39 + 39 個 placeholder PNG/JPG(charity logo、project / sale-item cover)。

backend application 本身為 spec 018 簽 URL 上傳已具備此權限,seed 復用同一個 role 即可,**無需新增 policy**。但首次 deploy 後務必確認 CloudWatch log 沒有 `AccessDenied: s3:PutObject` — 如有,代表 role policy 限定了 key prefix,需放寬至 `donation/charities/*` `donation/donation-projects/*` `donation/sale-items/*`。

### 改動清單

- `backend/package.json`:`tsx` 從 devDependencies → dependencies
- `backend/.github/workflows/pipeline.yml`:
  - step 名稱:`Run Prisma migrate` → `Run Prisma migrate + seed`
  - command override:加上 `&& npx prisma db seed`
  - error log 說明文字補上「migrate / seed 共用 exit code,靠 log 區分」
...
| 0.2 | 2026-06-14 | seed 步驟搭在同一個 one-shot task 上;tsx 移到 dependencies;明文 deploy = data-reset 的 demo-only 假設 |
